...
if ($total -eq 0) {
    ...
    foreach ($file in $videos) {        
        ...
        $output = Join-Path $PSScriptRoot "\output\$($file.BaseName)_1080_audio.mp4"
        ffmpeg -y -fflags +genpts -i "$($file.FullName)" `
                -c:v libx264 -preset veryslow -pix_fmt yuv420p -movflags +faststart -map_metadata 0 `
	       	    -vf "scale=-2:1080" -r 30 -crf 21 `
               "$output"
        
        $output = Join-Path $PSScriptRoot "\output\$($file.BaseName)_1440_audio.mp4"
        ffmpeg -y -fflags +genpts -i "$($file.FullName)" `
                -c:v libx264 -preset veryslow -pix_fmt yuv420p -movflags +faststart -map_metadata 0 `
	       	    -vf "scale=-2:1440" -r 30 -crf 21 `
               "$output"
        ...
        $output = Join-Path $PSScriptRoot "\output\$($file.BaseName)_1080_audio.webm"        
        ffmpeg -y -fflags +genpts -i "$($file.FullName)" `
                -c:v libvpx-vp9 -b:v 0 -speed 0 -map_metadata 0 `
	       	    -vf "scale=-2:1080" -r 30 -crf 34 `
               "$output"
        
        $output = Join-Path $PSScriptRoot "\output\$($file.BaseName)_1440_audio.webm"        
        ffmpeg -y -fflags +genpts -i "$($file.FullName)" `
                -c:v libvpx-vp9 -b:v 0 -speed 0 -map_metadata 0 `
	       	    -vf "scale=-2:1440" -r 30 -crf 34 `
               "$output"
