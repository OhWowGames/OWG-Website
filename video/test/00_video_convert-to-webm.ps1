...
$videos = Get-ChildItem -Path . -File | Where-Object { $_.Extension -match '\.(mp4|mkv|mov|avi|webm)$' }
...
if ($total -eq 0) {
    ...
    foreach ($file in $videos) {
        $output = Join-Path $PSScriptRoot "$($file.BaseName)_portrait.mp4"
        
        Write-Host "`n--------------------------------------------------" -ForegroundColor Cyan
        Write-Host "Processing ($current of $total): $($file.Name)" -ForegroundColor Yellow
        
        # FFmpeg Command: No audio (-an)frame rate(-r 60) (webm -c:v libvpx-vp9 -crf 34) (mp4 -crf 21) (only render 1 frame -vframes 30) (-vf "crop=1215:2160:0:0") (-vf "scale=trunc(oh*a/2)*2:1440")

        ffmpeg -i "$($file.FullName)" `
               	-crf 21 -b:v 0 -speed 0 `
               	-y -an -r 30 `
		-map_metadata 0 -fflags +genpts `
	       	-vf "crop=810:1440:1750:0" `
               "$output"
