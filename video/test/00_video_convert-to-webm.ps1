...
if ($total -eq 0) {
    ...
    foreach ($file in $videos) {
        $output = Join-Path $PSScriptRoot "$($file.BaseName)_1440.mp4"
        
        Write-Host "`n--------------------------------------------------" -ForegroundColor Cyan
        Write-Host "Processing ($current of $total): $($file.Name)" -ForegroundColor Yellow
        
        # FFmpeg Command: No audio (-an)frame rate(-r 60) (webm -c:v libvpx-vp9 -crf 34) (mp4 -crf 21) (-vf "scale=trunc(oh*a/2)*2:2160" `)

        ffmpeg -i "$($file.FullName)" `
               	-crf 21 -b:v 0 -speed 0 `
               	-y -an -r 30 -vf "scale=trunc(oh*a/2)*2:1440" `
